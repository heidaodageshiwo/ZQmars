...
$(function () {
    ...
    var imsiFollowTaskTable = $('#imsifollow-task-table').DataTable({
        ...
        "processing": true,
        "ajax": {
            "url": "/analysis/imsiFollow/getTasks",
            "dataType": "json"
        },
        "dataSrc": "",
        "columns": [
            ...
            },
            {
                "render" : function (data, type, row ) {
                    ...
                    if( data.taskProcess >= 100 ) {
                        dom = '<a href="/analysis/imsiFollow/report/' +  data.id +'" class="btn btn-flat btn-success btn-sm">查看</a>';
                        ...
                        dom = '<a href="/analysis/imsiFollow/report/' + data.id + '" class="btn btn-flat btn-success btn-sm disabled">查看</a>';
    ...
    $("#create-task-btn").click(function () {
        if( imsiFollowTaskFormValidate.form()) {
            ...
            var data = $("#imsiFollow-task-form").serialize();
            ...
            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: "/analysis/imsiFollow/createTask",
                data: data,
                timeout: 5000,
    ...
    $('#imsifollow-task-table tbody').on( 'click', 'tr a.export-task-btn', function () {
        var tr = $(this).closest('tr');
        var rowData = imsiFollowTaskTable.row( tr ).data();
