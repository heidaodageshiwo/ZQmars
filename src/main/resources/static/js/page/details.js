...
$(function () {
    ...
    function format ( d , rowIdx ) {
        ...
        var tableHeaderDom = $('<thead><th>设备编号</th><th>状态</th><th>移动上号数</th><th>联通上号数</th><th>电信上号数</th><th>未知运营商上号数</th><th>操作</th></thead>');
        tableHeaderDom.appendTo(tableDom);

        var tableBodyDom = $('<tbody></tbody>');
        /*style="color:green">' + device.zt +'</td><td>'*/
        if(  d.devices ) {
            $.each( d.devices, function ( i, device) {
                var tableTrDom = $('<tr ></tr>');
                var cc;
                if(device.zt=="在线"){
                    cc='<td style="color:green">' + device.zt +'</td>'
                }else{
                    cc='<td>' + device.zt +'</td>'
                }
                var tableTdDom = $('<td>'+ device.sn + '</td>'+cc
                    +'<td>'    + device.yd +'</td><td>'
                    + device.lt + '</td><td>'+ device.dx
                    + '</td><td>'+ device.wz + '</td>'
                    +'<td>'+

                        '<button data-idx="' + i + '" data-rowidx="' + rowIdx +  '"  class="btn btn-flat btn-sm btn-success site-device-edit-btn">查看</button>'
                    +'</td>'
   ...
   /* function format ( d , rowIdx ) {
    ...
    function exportDataProgress( exoprtInfo ) {
        ...
        $.ajax({
            type: "post",
            url: "/details/exportDataProgress",
            dataType:"json",
